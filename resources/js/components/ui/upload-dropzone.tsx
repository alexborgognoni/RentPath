...
export function UploadDropzone({ control: { upload, isPending }, id: _id, accept, metadata, description, uploadOverride }: UploadDropzoneProps) {
    ...
    return (
        <div
            className={cn('relative rounded-lg border border-dashed border-input text-foreground transition-colors', {
        ...
        >
            <label
                ...
                htmlFor={_id || id}
            >
                <div className="my-2">{isPending ? <Loader2 className="size-6 animate-spin" /> : <Upload className="size-6" />}</div>
